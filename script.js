 ...
 */
function buildWeatherPrompt(currentData, forecastData, aqiData, variation = 0) {
    ...
    return `Você é um meteorologista digital para um app brasileiro.
Escreva frases naturais, elegantes e úteis para cards pequenos. Use linguagem clara, sem exagero, sem piadas, sem clichês e sem "aproveite" solto.
Cada insight deve explicar o que está acontecendo e, quando fizer sentido, indicar uma ação simples. Use números reais dos dados abaixo; não invente medições.
...
{"insights":[{"icon":"emoji","category":"Resumo|Alerta|Saúde|Dica|Semana|Atividade","title":"até 28 caracteres","message":"até 115 caracteres"}]}

Gere EXATAMENTE 3 insights:
1. Resumo: leitura do clima atual em uma frase.
2. Alerta, Saúde ou Semana: o ponto mais importante para as próximas horas/dias.
3. Dica ou Atividade: recomendação prática para a rotina.

Regras de estilo:
- Fale com o usuário de forma direta e acolhedora.
- Evite frases genéricas como "fique atento", "planeje-se bem" ou "tudo sob controle" sem contexto.
- Não repita a mesma informação nos três cards.
- Não coloque emoji dentro de title ou message; use emoji apenas no campo icon.
- Variação solicitada: ${variation}. Se for maior que 0, reescreva os cards com palavras e ângulos diferentes da análise anterior, mantendo os mesmos dados.`;
}

/**
 * Calls the Gemini REST API using x-goog-api-key header (required for AQ. keys).
 */
async function analyzeWeatherWithAI(currentData, forecastData, aqiData, variation = 0) {
    ...
    try {
        const prompt = buildWeatherPrompt(currentData, forecastData, aqiData, variation);
        ...
        const response = await fetch(GEMINI_API_URL, {
            ...
            body: JSON.stringify({
                ...
                generationConfig: {
                    temperature: variation > 0 ? 0.65 : 0.45,
                    topP: 0.9,
        ...
        try {
            ...
            const fallbackResponse = window.generateLocalFallbackInsights(currentData, forecastData, aqiData, variation);
...
function renderAIInsights(insights) {
    ...
    const categoryClass = {
        ...
        'previsao': 'ai-cat-semana',
        'previsão': 'ai-cat-semana',
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (refreshBtn) {
        refreshBtn.addEventListener('click', () => {
            if (!currentResponseData) return;
            window.aiAnalysisRefreshSeed = (window.aiAnalysisRefreshSeed || 0) + 1;
            refreshBtn.classList.add('spinning');
            analyzeWeatherWithAI(currentResponseData, currentChartData, currentAqiData, window.aiAnalysisRefreshSeed)
